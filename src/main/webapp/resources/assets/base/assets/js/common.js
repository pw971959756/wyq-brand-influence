...
"hideMethod": "fadeOut"
	...
	$(".keywordBox").on("click", "li", function() {
		var text1 = "<div class='token'><span class='token-label' style='max-width: 93px;'>";
		var text2 = $(this).text();
		var text3 = "</span><a href='javascript:;' class='close' tabindex='-1'>×</a></div>";
		$(this).addClass("active")
		$(this).parents(".form-group").find(".tokenfield .token-input").before(text1 + text2 + text3);
		$(this).parents(".form-group").find(".tokenfield .token").on("click", ".close", function(i) {
			$(this).parents(".token").remove();
		});
	});

	//公共弹出框组件
	$(".showToolBtn").on("click", function(e) {
		$(".toolBox").hide("")
		$(this).parents(".showTool").next(".toolBox").show("")
		e.stopPropagation();
	});
	$(".showToolClick").on("click", function(e) {
		e.stopPropagation();
	})
	...
	//选择类型
	$(".blocks-choice li").on("click", function() {
		$(this).toggleClass("active")
	});

	//搜索条件设置
	$(".setGroup .form-horizontal .group-input button.btn").on("click", function() {
		$(this).parents(".group-input").find(".btn").removeClass("btn-primary")
...
//关键词选择效果
$.components.register("select2",{mode:"init",defaults:{placeholder: '',width:"style",theme:"material"},init:function(context){if($.fn.select2){var defaults=$.components.getDefaults("select2");$('[data-plugin="select2"]',context).each(function(){var options=$.extend({},defaults,$(this).data());$(this).select2(options)})}}});
...
! function(document, window, $) {
	...
	! function() {
		var largeContent = $("#termCondition1").html(),
			largeSettings = {
				title: "WebUI Popover",
				content: largeContent,
				width: 400
			};
		$("#termPOP1").webuiPopover($.extend({}, defaults, largeSettings))
	}(),
	function() {
		var largeContent = $("#termCondition2").html(),
			largeSettings = {
				title: "WebUI Popover",
				content: largeContent,
				width: 400
			};
		$("#termPOP2").webuiPopover($.extend({}, defaults, largeSettings))
	}(),
	function() {
		var largeContent = $("#termCondition3").html(),
			largeSettings = {
				title: "WebUI Popover",
				content: largeContent,
				width: 400
			};
		$("#termPOP3").webuiPopover($.extend({}, defaults, largeSettings))
	}(),
	function() {
		var largeContent = $("#termCondition4").html(),
			largeSettings = {
				title: "WebUI Popover",
				content: largeContent,
				width: 400
			};
		$("#termPOP4").webuiPopover($.extend({}, defaults, largeSettings))
	}(),
	function() {
		var largeContent = $("#termCondition5").html(),
			largeSettings = {
				title: "WebUI Popover",
				content: largeContent,
				width: 400
			};
		$("#termPOP5").webuiPopover($.extend({}, defaults, largeSettings))
	}(),
	function() {
		var largeContent = $("#termCondition6").html(),
			largeSettings = {
				title: "WebUI Popover",
				content: largeContent,
				width: 400
			};
		$("#termPOP6").webuiPopover($.extend({}, defaults, largeSettings))
	}()
